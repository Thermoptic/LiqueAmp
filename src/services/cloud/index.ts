...
export async function createCloudServices(env?: Record<string, string | undefined>): Promise<CloudServices> {
  const local: CloudServices = { provider: createLocalAccountProvider(), cloud: null, problem: null };
  let config;
  try {
  ...
  try {
    ...
    return { provider: createSupabaseAccountProvider(client, { redirectTo: () => authCallbackUrl() }), cloud: createSupabaseProfileStore(client), problem: null };
  } catch (err) {
